...
if (x%%2 !=0){
 median.odd<-x
 Median.odd<-sort(median.odd)
 return(Median.odd)}
 else{
 median.even<-x
 Median.even<-sort(median.even)
 return(Median.even)
 }
}
x<-c(1,2,3,4,5)
...
#if x is not divisible by 2, then store x into this object 
example.odd<-c(1,2,3,4,5)
